...
def rpn_cls_loss():
    def _rpn_cls_loss(y_true, y_pred):
        ...
        # -1 忽略, 0 是背景, 1 是存在目标
        anchor_state   = y_true 
        indices_for_no_ignore        = tf.where(K.not_equal(anchor_state, -1))
        ...
        cls_loss_for_no_ignore = K.binary_crossentropy(labels_for_no_ignore, classification_for_no_ignore)
        cls_loss_for_no_ignore = K.sum(cls_loss_for_no_ignore)

        # 标准化
        normalizer_no_ignore = tf.where(K.not_equal(anchor_state, -1))
        normalizer_no_ignore = K.cast(K.shape(normalizer_no_ignore)[0], K.floatx())
